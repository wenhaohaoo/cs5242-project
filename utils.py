...
def extract(fn):
    ...
    for line in lines:
        ...
        elif "error rate on test set =" in line:
            test_error.append(float(line.split("=")[1].split()[0]))

    return loss, error, test_error


def plot_optimizers(kw):
    loss = {}
    error = {}
    test_error = {}
    fn_name = {
        "optimizer_sgd.txt": "SGD",
        "optimizer_sgd_momentum.txt": "momentum SGD",
        "optimizer_adam.txt": "Adam"
    }
    for fn, name in fn_name.items():
        loss_, error_, test_error_ = extract('./log/' + fn)
        loss[name] = loss_
        error[name] = error_
        test_error[name] = test_error_
